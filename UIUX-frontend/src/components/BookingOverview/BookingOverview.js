...
const BookingOverview = (props) => {
    ...
    const ParticipantNames = (props) => {
        ...
        useEffect(() => {
            (async function () {
                ...
                let response = await axios.get(
                    apiUrl + `/bookings/details/${bookingId}`
                );
                ...
                for (const bookingsDetail of bookingDetails) {
                    response = await axios.get(
                        apiUrl + `/users/${bookingsDetail.user_id}`
                    );
        ...
        bookingParticipants.forEach((participant) => {
            list.push(<p>{participant}</p>);
        });

        return (
            <div>
                <p>
                    <b>Participants</b>: {list}
                </p>
            </div>
        );
    };
    ...
    return (
        <div>
            ...
            <div className="booking-overview m-4">
                ...
                <div>
                    <p>
                        <b>Room name</b>: {props.room}
                    </p>
                    <p>
                        <b>Time</b>:{" "}
                        {hours >= 1 ? <span>{hours} hours </span> : null}{" "}
